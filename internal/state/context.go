...
func (ctx *Context) ReplyEmbed(embed *discordgo.MessageEmbed) {
        if ctx.SourceType == SourceTypeInteraction && !ctx.InteractionResponded {
                ctx.Session.InteractionRespond(ctx.Interaction.Interaction, &discordgo.InteractionResponse{
                        Type: discordgo.InteractionResponseChannelMessageWithSource,
                        Data: &discordgo.InteractionResponseData{
...
func (ctx *Context) determineArgumentsFromMessage() {
        ...
        case "search", "buscar":
                cmdLen := 8
                if ctx.CommandName == "buscar" {
                        cmdLen = 8
                }
                if len(ctx.Message.Content) > cmdLen {
                        ctx.ArgumentsRaw["query"] = ctx.Message.Content[cmdLen:]
                } else {
                        ctx.ArgumentsRaw["query"] = ""
                }
        case "volume", "volumen":
                cmdLen := 8
                if ctx.CommandName == "volumen" {
                        cmdLen = 9
                }
                if len(ctx.Message.Content) > cmdLen {
                        ctx.ArgumentsRaw["level"] = ctx.Message.Content[cmdLen:]
                } else {
                        ctx.ArgumentsRaw["level"] = ""
                }
        case "nuke", "limpiar":
                cmdLen := 6
                if ctx.CommandName == "limpiar" {
                        cmdLen = 9
                }
                if len(ctx.Message.Content) > cmdLen {
                        ctx.ArgumentsRaw["count"] = ctx.Message.Content[cmdLen:]
                } else {
